...
/** Radix `DropdownMenuTrigger asChild` requires a ref-forwarding child; the package `Button` does not forward refs. */
...
>(({ className, ...props }, ref) => (
  <button
    ...
    type="button"
    className={cn(
      "inline-flex size-9 shrink-0 items-center justify-center rounded-full text-foreground outline-none hover:bg-accent focus-visible:ring-2 focus-visible:ring-ring",
      className
    )}
    {...props}
  />
...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="relative flex items-center justify-between w-full py-4 px-4 md:px-16 lg:px-24 xl:px-32 backdrop-blur border-b text-white border-slate-800">
        <Link to='/'>
          ...
          <div className="flex items-center gap-3">
            {isPending ? (
              <div
                className="h-9 min-w-[7.5rem] rounded bg-white/10 animate-pulse"
                aria-busy="true"
              ...
              <div className="relative z-[9999]">
                <UserButton
                  trigger={
                    <UserMenuTrigger>
                      <UserAvatar user={session.user} isPending={false} />
                    </UserMenuTrigger>
                  }
                />
              </div>
        ...
        {menuOpen && (
          <div className="fixed inset-0 z-[100] bg-black/60 text-white backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-300">
            ...
            <Link to='/community' onClick={() => setMenuOpen(false)}>Communioty</Link>
